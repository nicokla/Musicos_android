...
// java threads:
// run, wait, notifyAll, interrupt
// synchronized, ReentrantLock (mutex), volatile, AtomicBoolean (?)
// https://jcip.net/listings.html
// http://blog.paumard.org/cours/java-api/chap05-concurrent-atomicite.html
// https://www.jmdoudoux.fr/java/dej/chap-acces_concurrents.htm#acces_concurrents-1
...
public class SequencerImpl implements Sequencer {
    ...
    private float tempoInBPM = 60.0f;
    private long songLengthInTicks = 60;
    public ReentrantLock mutex = new ReentrantLock(); // for tickPosition
    public ReentrantLock mutex2 = new ReentrantLock(); // for tickPositionSetTime

    // volatile so supposedly no need for mutex in java.
    ...
    public class SequencerThread extends Thread {
        ...
        // playing
        public Track playingTrack = null;
        public long tickPositionSetTime;
//        public long runningStoppedTime;
        public volatile boolean needRefreshPlayingTrack = false;
        // recording
        ...
        public SequencerThread() {
            ...
            midiEventRecordingReceiver = new Receiver() {
                ...
                // !!! on ajoute l'event au recordingTrack
                @Override
                public void send(@NonNull final MidiMessage message, final long timeStamp) {
                    ...
                    Log.d("time", String.valueOf((long) (recordStartedTick + ((System.currentTimeMillis() - recordingStartedTime) * 1000.0f * getTicksPerMicrosecond()))));
...
//            setTickPosition(getTickPosition());
            try {
                ...
                try{
                    mutex2.lock();
                    tickPosition = (long) (tickPosition + ((System.currentTimeMillis() - tickPositionSetTime) * 1000.0f * getTicksPerMicrosecond()));
                } finally{
                    mutex2.unlock();
                }
            } finally {
        ...
        public long getTickPosition() {
            ...
            try {
                ...
                if (isRunning) {
                    try {
                        mutex2.lock();
                        answer = (long) (tickPosition + ((System.currentTimeMillis() - tickPositionSetTime) * 1000.0f * getTicksPerMicrosecond()));
                    }finally{
                        mutex2.unlock();
                    }
//                    Utils.print("aaaaa\n");
...
//                Utils.print(answer);
        ...
        public void setTickPosition(final long tick) {
            try {
                ...
                tickPosition = tick;
                if (isRunning) {
                    setTickPositionSetTime(System.currentTimeMillis());
                }
            } finally {
                mutex.unlock();
            }
        ...
        public void setTickPositionSetTime(final long tick){
            try {
                mutex2.lock();
                tickPositionSetTime = tick;
            } finally {
                mutex2.unlock();
            }
        }
       ...
       // !!!
        public void startRecording() {
            ...
            System.out.println("!!!");
            System.out.println(getTickPosition());
...
//            setTickPosition(getTickPosition());
        ...
        public void fireEventListeners(@NonNull final MidiMessage message) {
            if (message instanceof MetaMessage) {
                ...
                Log.d("hello!!", shortMessage.toString());
            ...
            while (isOpen) {
                ...
                for (int loop = 0; loop < getLoopCount() + 1; loop = (getLoopCount() == LOOP_CONTINUOUSLY ? loop : loop + 1)) {
                    ...
                    for (int i = 0; i < playingTrack.size(); i++) {
                        ...
                        if (needRefreshPlayingTrack) {
                            // skip to lastTick
                            if (midiEvent.getTick() < getTickPosition()) {
...
//                            // outer loop
////                            setTickPosition(midiEvent.getTick());
//                            continue;
//                        }

                        try {
                            final long sleepLength = (long) ((1.0f / getTicksPerMicrosecond()) * (midiEvent.getTick() - getTickPosition()) / 1000f / getTempoFactor());
                            if (sleepLength > 0) {
                                sleep(sleepLength);
                            }
                            System.out.println("!!!\n");
                            System.out.println(midiEvent.getMessage());
                            setTickPosition(midiEvent.getTick());
...
//                runningStoppedTime = System.currentTimeMillis();
        ...
        public boolean processTempoChange(@NonNull final MetaMessage metaMessage) {
        ...
        public void refreshPlayingTrack() {
            ...
            if (tracks.length > 0) {
                try {
                    ...
                    playingTrack = TrackUtils.mergeSequenceToTrack(SequencerImpl.this, recordEnable, songLengthInTicks);
    ...
    public SequencerImpl(long songLengthInSeconds, MidiPlayer midiPlayer) {
        try {
            sequence = new Sequence(Sequence.PPQ, 120);
        } catch (InvalidMidiDataException e) {
            e.printStackTrace();
        }
        this.songLengthInTicks = songLengthInSeconds * sequence.getResolution();
        try {
            this.open(); // now thread exists, so midiEventRecordingReceiver also.
        } catch (MidiUnavailableException e) {
            e.printStackTrace();
        }
        this.midiPlayer = midiPlayer;
//        return sequencerThread.midiEventRecordingReceiver;
    }
    ...
    public void clear(){
        ...
        if (sequencerThread.playingTrack != null) {
            sequencerThread.playingTrack.clear();
        }
        if (sequencerThread.recordingTrack != null) {
            sequencerThread.recordingTrack.clear();
        }
    }
     ...
     */
    public float getTicksPerMicrosecond() {
        if (sequence == null) {
            return Float.NaN;
        }
